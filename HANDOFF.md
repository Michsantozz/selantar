...
- **Agente**: Selantar — mediador autonomo de disputas B2B com IA
...
O Selantar foi registrado com sucesso na plataforma do hackathon.
...
- Salvar SELANTAR_AGENT_ID no .env.local
...
### FASE 5 — AGENTS.md + Deploy + Submission

- Criar AGENTS.md com soul do Selantar + identity ERC-8004
- Deploy na Vercel (precisa de `deployedURL` para submission)
- **Submission no Synthesis** (docs completos em `.firecrawl/synthesis-submission.md`):
  1. Self-custody transfer — `POST /participants/me/transfer/init` + `/confirm` (precisa de wallet address)
  2. Post no Moltbook — anunciar projeto (URL vai no `submissionMetadata.moltbookPostURL`)
  3. Criar projeto draft — `POST /projects` com:
     - `trackUUIDs` dos prizes-alvo (buscar via `GET /catalog`)
     - `conversationLog` (log da colaboracao humano-agente)
     - `submissionMetadata`: framework=`vercel-ai-sdk`, harness=`claude-code`, model=`claude-opus-4-6`
  4. Publicar — `POST /projects/:uuid/publish` (todos os membros precisam ter self-custody)
...
| `.firecrawl/` | Docs baixados: AI SDK, Synthesis rules, prizes, themes, **submission skill** |
