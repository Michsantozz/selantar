...
export const GanttContentHeader: FC<GanttContentHeaderProps> = ({
  ...
  return (
    ...
    >
      <div>
        <div
          className="sticky inline-flex whitespace-nowrap px-3 py-2 font-medium uppercase tracking-wider text-muted-foreground text-xs"
          style={{
      ...
      >
        {Array.from({ length: columns }).map((_, index) => (
          <div
            className="shrink-0 border-border border-b py-1 text-center font-mono text-xs text-muted-foreground"
            key={`${id}-${index}`}
          >
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative flex items-center gap-2.5 p-2.5 text-xs transition-colors hover:bg-muted/50",
        className
      )}
      key={feature.id}
      onClick={handleClick}
    ...
    >
      ...
      />
      <p className="pointer-events-none flex-1 truncate text-left font-medium text-foreground">
        {feature.name}
      </p>
      <p className="pointer-events-none font-mono text-muted-foreground">{duration}</p>
    </div>
  );
};
...
export const GanttSidebarHeader: FC = () => (
  <div
    className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-border border-b bg-card/90 p-2.5 font-medium uppercase tracking-wider text-muted-foreground text-xs backdrop-blur-sm"
    style={{ height: "var(--gantt-header-height)" }}
  >
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    <p
      className="w-full truncate p-2.5 text-left font-medium uppercase tracking-wider text-muted-foreground text-xs"
...
export const GanttSidebar: FC<GanttSidebarProps> = ({
  ...
  <div
    className={cn(
      "sticky left-0 z-30 h-max min-h-full overflow-clip border-border border-r bg-card/90 backdrop-blur-md",
      className
    )}
  ...
  return (
    ...
    >
      ...
      {date && (
        <div
          className={cn(
            "-translate-x-1/2 absolute top-10 hidden whitespace-nowrap rounded-md border border-border bg-popover px-2 py-1 font-mono text-foreground text-xs backdrop-blur-lg group-hover:block",
            isPressed && "block"
          )}
        >
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card className="h-full w-full rounded-md border-border bg-muted p-2 text-xs shadow-none">
      <div
        className={cn(
    ...
    >
      <ContextMenu>
        <ContextMenuTrigger render={<div className={cn(
                            "group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md border border-border bg-card px-2 py-1 font-medium uppercase tracking-wider text-foreground text-xs",
                            className
                          )} />}>{label}<span className="max-h-[0] overflow-hidden font-mono normal-case tracking-normal opacity-80 transition-all group-hover:max-h-[2rem]">
                            {formatDate(date, "MMM dd, yyyy")}
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "gantt relative isolate grid h-full w-full flex-none select-none overflow-auto rounded-xl border border-border bg-card",
          range,
          className
        )}
        ref={scrollRef}
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md border border-accent/30 bg-accent/10 px-2 py-1 font-medium uppercase tracking-wider text-accent text-xs",
          className
        )}
      >
        {label}
        <span className="max-h-[0] overflow-hidden font-mono normal-case tracking-normal opacity-80 transition-all group-hover:max-h-[2rem]">
          {formatDate(date, "MMM dd, yyyy")}
        </span>
      </div>
      <div className={cn("h-full w-px bg-accent/40", className)} />
    </div>
  );
};
